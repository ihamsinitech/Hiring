...
const CompanyDashBoard = () => {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      fetch(`https://www.careerspott.com/api/auth/company/${storedUser.userId}/profile`)
        .then(res => {
          if (!res.ok) {
  ...
  useEffect(() => {
    const storedUser = JSON.parse(localStorage.getItem("userData"));
    if (storedUser) {
      fetch(`https://www.careerspott.com/api/auth/company/${storedUser.userId}/jobs`)
        .then(res => {
          if (!res.ok) {
  ...
  useEffect(() => {
    const storedUser = JSON.parse(localStorage.getItem("userData"));
    if (storedUser) {
      fetch(`https://www.careerspott.com/api/auth/company/${storedUser.userId}/jobs-with-applications`)
        .then(res => {
          if (!res.ok) {
            throw new Error('Jobs not found');
          }
